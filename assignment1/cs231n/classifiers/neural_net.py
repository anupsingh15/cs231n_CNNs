...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    Z = f(a)
    scores = Z.dot(W2) + b2
    #scores = (f(X.dot(W1) + b1).dot(W2))+b2
    ...
    for i, x in enumerate(X):
        dL_dS = prob[i][:, np.newaxis]
        dL_dS[y[i]] -= 1
        dL_dW2 += np.dot(Z[i].T[:, np.newaxis], dL_dS.T)
        dL_db2 += dL_dS
        dL_dY = np.dot(W2, dL_dS)
        dL_dY[a[i] <= 0] = 0
        dL_dW1 += np.dot(X[i].T[:, np.newaxis], dL_dY.T)
        dL_db1 += dL_dY
        #print (a[i], dL_dY)   
    ...
    dL_dW2 += reg*W2
    dL_dW1 += reg*W1
    dL_db1 = dL_db1.reshape(dL_db1.shape[0],)
    dL_db2 = dL_db2.reshape(dL_db2.shape[0],)
    grads['W2'] = dL_dW2
    grads['b2'] = dL_db2
    grads['W1'] = dL_dW1
    grads['b1'] = dL_db1
